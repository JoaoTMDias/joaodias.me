...
const Meta: React.FunctionComponent<IMetaProps> = props => {
  ...
  const mainTitle: string = `${config.title} - Designer and Developer`;
  const metaTitle: string = title ? `${props.title} | ${mainTitle}` : mainTitle;
  const metaDescription: string = description || config.description;
  const absoluteUrl: string = `${config.url}${location.pathname}`;
